...
const kepegawaian_pns = (db) => {
  ...
  const getDataUnduhPejabatStruktural = (qwhere) => {
    const query = db.any(
      "SELECT kpns.id, kpns.nama, kpns.kepegawaian_nip, kpns.kepegawaian_nrk, mj.nama as kepegawaian_jabatan, kpns.kepegawaian_tempat_tugas, '' as kepegawaian_keterangan_pejabat_struktural FROM kepegawaian_pns kpns LEFT JOIN master_jabatan mj ON mj.id = kpns.kepegawaian_jabatan WHERE kpns.is_deleted = 0" + qwhere
    );
  ...
  const updatePensiun = (keterangan_pensiun) => {
    const query = db.one(
      ...
      [keterangan_pensiun, nomor, no_pegawai, nama, kepegawaian_jabatan,
