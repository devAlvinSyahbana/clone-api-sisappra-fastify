...
const kepegawaian_rekapitulasi = (db) => {
  ...
  const find_rekapitulasi_jft = (limit, offset, nama, nrk, jabatan, tempat_tugas, seksi_kecamatan, kelurahan) => {
    ...
    if (tempat_tugas != undefined) {
      filter = filter + " and kp.kepegawaian_tempat_tugas ilike '" + "%" + tempat_tugas + "%" + "'"
    }
    if (seksi_kecamatan != undefined) {
      filter = filter + " and kp.kepegawaian_tempat_tugas ilike '" + "%" + seksi_kecamatan + "%" + "'"
    }
    if (kelurahan != undefined) {
      filter = filter + " and kp.kepegawaian_kelurahan ilike '" + "%" + kelurahan + "%" + "'"
    }
    if (nama != undefined) {
      filter = filter + " and kp.nama ilike '" + "%" + nama + "%" + "'"
    }
    if (nrk != undefined) {
      filter = filter + " and kp.kepegawaian_nrk ilike '" + "%" + nrk + "%" + "'"
    }
  ...
  const find_rekapitulasi_kenaikan_pangkat = (limit, offset, nama, nrk, nip, jabatan, pangkat, status_kenaikan_pangkat, tempat_tugas, seksi_kecamatan, kelurahan) => {
    ...
    if (seksi_kecamatan != undefined) {
      filter = filter + " and kp.kepegawaian_subbag_seksi_kecamatan ilike '" + "%" + seksi_kecamatan + "%" + "'"
    }
    if (kelurahan != undefined) {
      filter = filter + " and kp.kepegawaian_kelurahan ilike '" + "%" + kelurahan + "%" + "'"
    }
    ...
    if (nip != undefined) {
      filter = filter + " and kp.kepegawaian_nip ilike '" + "%" + nip + "%" + "'"
    }
    if (jabatan != undefined) {
      filter = filter + " and kp.kepegawaian_jabatan = " + jabatan
    }
    if (status_kenaikan_pangkat != undefined) {
      filter = filter + " and kp.status_kenaikan_pangkat = " + status_kenaikan_pangkat
    }
  ...
  const find_duk_rekapitulasi_pegawai = (nama, nip, nrk_nptt_pjlp, status_pegawai, tempat_tugas, seksi_kecamatan, kelurahan, limit, offset) => {
    let filter_1 = ""
    let filter_2 = ""
    ...
    const query = db.any(
      "select z.* from (select kp.id, kp.nama, kp.kepegawaian_nip as nip, kp.kepegawaian_nrk as nrk_nptt_npjlp, mj.nama as jabatan, kp.kepegawaian_status_pegawai as status_pegawai, kp.kepegawaian_tempat_tugas as tempat_tugas,  to_char( kp.tgl_lahir, 'DD-MM-YYYY') as tanggal_lahir, ma.nama as agama, kp.domisili_alamat as alamat from kepegawaian_pns kp left join master_jabatan mj on kp.kepegawaian_jabatan = mj.id left join master_agama ma on kp.agama = ma.id where kp.is_deleted = 0" +
      filter_1 + "union all select knp.id, knp.nama, knp.kepegawaian_nip, knp.kepegawaian_nptt_npjlp,  mj2.nama as jabatan, knp.kepegawaian_status_pegawai , knp.kepegawaian_tempat_tugas , to_char( kNp.tgl_lahir, 'DD-MM-YYYY') as tanggal_lahir, ma2.nama as agama, knp.domisili_alamat  from kepegawaian_non_pns knp left join master_jabatan mj2 on knp.kepegawaian_jabatan = mj2.id left join master_agama ma2 on knp.agama = ma2.id where knp.is_deleted = 0" + filter_2 +
  ...
  const count_duk_rekapitulasi_pegawai = (nama, nip, nrk_nptt_pjlp, status_pegawai, tempat_tugas, seksi_kecamatan, kelurahan) => {
    let filter_1 = ""
    let filter_2 = ""
  ...
  // ^ find pensiun
  const PensiunFindFilter = (nama, nrk_nptt_pjlp, tempat_tugas, seksi_kecamatan, status_pegawai, tahun_pensiun, limit, offset) => {
    let filter_1 = ""
    let filter_2 = ""
    ...
    if (tahun_pensiun != undefined) {
      filter_pensiun_1 = filter_pensiun_1 + " z.tahun_pensiun = " + tahun_pensiun;
      filter_pensiun_2 = filter_pensiun_2 + " x.tahun_pensiun = " + tahun_pensiun;
    } else {
      filter_pensiun_1 = filter_pensiun_1 + " z.tahun_pensiun = date_part('year', now()) or z.tahun_pensiun = (date_part('year', now()) + 1)";
      filter_pensiun_2 = filter_pensiun_2 + " x.tahun_pensiun = date_part('year', now()) or x.tahun_pensiun = (date_part('year', now()) + 1)";
    }
    console.log(filter_1)
    console.log(filter_2)
    const query = db.any(
      "select y.* from (select z.*from (select kp.nama, kp.kepegawaian_nip as nip,kp.kepegawaian_nrk as no_pegawai,kp.kepegawaian_status_pegawai,mj.nama as jabatan,kp.kepegawaian_tempat_tugas as tempat_tugas,kp.kepegawaian_subbag_seksi_kecamatan as subbag_seksi_kecamatan, kp.tempat_lahir,kp.tgl_lahir,case when kp.kepegawaian_eselon = 1 or kp.kepegawaian_eselon = 2 then (date_part('year', kp.tgl_lahir) + 60) else  (date_part('year', kp.tgl_lahir) + 58) end as tahun_pensiun from kepegawaian_pns kp left join master_jabatan mj on kp.kepegawaian_jabatan = mj.id where kp.is_deleted = 0 " + filter_1 + " ) as z where " + filter_pensiun_1 + " union select x.*from (select knp.nama, knp.kepegawaian_nip,knp.kepegawaian_nptt_npjlp,knp.kepegawaian_status_pegawai,mj2.nama,knp.kepegawaian_tempat_tugas,knp.kepegawaian_subbag_seksi_kecamatan,knp.tempat_lahir,knp.tgl_lahir,(date_part('year', knp.tgl_lahir) + 56) as tahun_pensiun from kepegawaian_non_pns knp left join master_jabatan mj2 on knp.kepegawaian_jabatan = mj2.id where knp.is_deleted = 0 " + filter_2 + " ) as x where " + filter_pensiun_2 + ") as y ORDER BY tahun_pensiun asc LIMIT " +
      limit +
      " OFFSET " +
      (parseInt(offset) - 1)
    );
  ...
  const PensiunCount = (nama, nrk_nptt_pjlp, tempat_tugas, seksi_kecamatan, status_pegawai, tahun_pensiun) => {
    let filter_1 = ""
    let filter_2 = ""
    let filter_pensiun_1 = ""
    let filter_pensiun_2 = ""
    ...
    const query = db.one(
      "select count(y.*) from (select z.*from (select kp.nama, kp.kepegawaian_nip as nip,kp.kepegawaian_nrk as no_pegawai,kp.kepegawaian_status_pegawai,mj.nama as jabatan,kp.kepegawaian_tempat_tugas as tempat_tugas,kp.kepegawaian_subbag_seksi_kecamatan as subbag_seksi_kecamatan, kp.tempat_lahir,kp.tgl_lahir,case when kp.kepegawaian_eselon = 1 or kp.kepegawaian_eselon = 2 then (date_part('year', kp.tgl_lahir) + 60) else  (date_part('year', kp.tgl_lahir) + 58) end as tahun_pensiun from kepegawaian_pns kp left join master_jabatan mj on kp.kepegawaian_jabatan = mj.id where kp.is_deleted = 0 " + filter_1 + " ) as z where " + filter_pensiun_1 + " union all select x.*from (select knp.nama, knp.kepegawaian_nip,knp.kepegawaian_nptt_npjlp,knp.kepegawaian_status_pegawai,mj2.nama,knp.kepegawaian_tempat_tugas,knp.kepegawaian_subbag_seksi_kecamatan,knp.tempat_lahir,knp.tgl_lahir,(date_part('year', knp.tgl_lahir) + 56) as tahun_pensiun from kepegawaian_non_pns knp left join master_jabatan mj2 on knp.kepegawaian_jabatan = mj2.id where knp.is_deleted = 0 " + filter_2 + " ) as x where " + filter_pensiun_2 + ") as y"
    );

    return query;
  };

  const PensiunUnduh = (nama, nrk_nptt_pjlp, tempat_tugas, seksi_kecamatan, status_pegawai, tahun_pensiun) => {
    let filter_1 = ""
    let filter_2 = ""
    let filter_pensiun_1 = ""
    let filter_pensiun_2 = ""
