...
const hirarki = (db) => {
  ...
  const find = () => {
    const query = db.any(
      "SELECT h.id, parentid, j.nama as position_name, '' as team FROM public.hirarki h left join master_jabatan_coba j on h.position_name = j.id order by h.parentid",
    );
